...
uninstall:
	rm $(DESTDIR)$(PREFIX)/bin/media-automount
	rm $(DESTDIR)$(PREFIX)/lib/udev/rules.d/99-media-automount.rules
	rm $(DESTDIR)$(PREFIX)/lib/systemd/system/media-automount@.service
	rm -fr $(DESTDIR)/etc/media-automount.d/
